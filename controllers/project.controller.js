...
exports.radar = async (req, res) => {
  ...
  try {
    ...
    // Fetch phase names, company headline, first tag name per slot, and all tags on projects
    const [phaseRows, company, tagSlotLabels, allProjectTags] = await Promise.all([
      ...
      db.sequelize.query(
        `SELECT DISTINCT t.id, t.tag_name, t.tag_color
         FROM tags t
         WHERE t.id IN (
           SELECT tag_1 FROM projects WHERE company_id_fk = ? AND tag_1 IS NOT NULL AND tag_1 != 0
           UNION
           SELECT tag_2 FROM projects WHERE company_id_fk = ? AND tag_2 IS NOT NULL AND tag_2 != 0
           UNION
           SELECT tag_3 FROM projects WHERE company_id_fk = ? AND tag_3 IS NOT NULL AND tag_3 != 0
         )
         ORDER BY t.tag_name`,
        { replacements: [companyId, companyId, companyId], type: db.Sequelize.QueryTypes.SELECT }
      ),
...
exports.progress = async (req, res) => {
  const companyId = req.session.company.id;
  try {
    const filterStart = req.session.radarFilterStart || null;
    const filterEnd   = req.session.radarFilterEnd   || null;

    const whereClause = { company_id_fk: companyId };
...
exports.countProjectsByTag1 = async (req, res) => {
  ...
  try {
    ...
    const tag1CountsWithNames = tag1Counts.map((tag) => {
      const tagInfo = tag1Names.find((t) => t.id === tag.tag_1);
      return {
        tag_1: tag.tag_1,
        tag_name: tagInfo ? tagInfo.tag_name : null,
        tag_color: tagInfo ? tagInfo.tag_color : null,
...
exports.countCostsByTag1 = async (req, res) => {
  ...
  try {
    ...
    const tagMap = Object.fromEntries(tagRows.map((t) => [t.id, { tag_name: t.tag_name, tag_color: t.tag_color }]));

    const result = tag1Costs.map((r) => ({
      tag_1: r.tag_1,
      tag_name: tagMap[r.tag_1]?.tag_name || null,
      tag_color: tagMap[r.tag_1]?.tag_color || null,
...
exports.countProjectsByTag2 = async (req, res) => {
  ...
  try {
    ...
    const tag2CountsWithNames = tag2Counts.map((tag) => {
      const tagInfo = tag2Names.find((t) => t.id === tag.tag_2);
      return {
        tag_2: tag.tag_2,
        tag_name: tagInfo ? tagInfo.tag_name : null,
        tag_color: tagInfo ? tagInfo.tag_color : null,
...
exports.countCostsByTag2 = async (req, res) => {
  ...
  try {
    ...
    const result = tag2Costs.map((r) => ({
      tag_2: r.tag_2,
      tag_name: tagMap[r.tag_2]?.tag_name || null,
      tag_color: tagMap[r.tag_2]?.tag_color || null,
      total_cost: parseFloat(r.total_cost),
    }));
...
exports.countProjectsByTag3 = async (req, res) => {
  ...
  try {
    ...
    const tag3CountsWithNames = tag3Counts.map((tag) => {
      const tagInfo = tag3Names.find((t) => t.id === tag.tag_3);
      return {
        tag_3: tag.tag_3,
        tag_name: tagInfo ? tagInfo.tag_name : null,
        tag_color: tagInfo ? tagInfo.tag_color : null,
...
exports.countCostsByTag3 = async (req, res) => {
  ...
  try {
    ...
    const result = tag3Costs.map((r) => ({
      tag_3: r.tag_3,
      tag_name: tagMap[r.tag_3]?.tag_name || null,
      tag_color: tagMap[r.tag_3]?.tag_color || null,
      total_cost: parseFloat(r.total_cost),
    }));
...
exports.ganttChart = async (req, res) => {
  ...
  const filterStart = req.session.flightviewFilterStart || null;
  const filterEnd   = req.session.flightviewFilterEnd   || null;
